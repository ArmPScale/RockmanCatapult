...
{    class PracticeScreen : Background
    {
        public bool isClicked = false;
        Color backButtonColor;

        public PracticeScreen(Texture2D[] texture)
            : base(texture)
        {
        }
        ...
        public override void Update(GameTime gameTime, List<Sprite> sprites)
        {
            ...
            switch (Singleton.Instance.CurrentMenuState)
            {
                case Singleton.MenuState.MainMenu:
                    alpha = 255;
                    break;
                case Singleton.MenuState.Practice:
                    //Escape
            ...
            switch (Singleton.Instance.CurrentScreenState)
            {
                case Singleton.ScreenState.MenuScreen:
                    switch (Singleton.Instance.CurrentMenuState)
                    {
                        case Singleton.MenuState.Practice:
                            //drawTextPractice
                            spriteBatch.DrawString(Singleton.Instance._font, "Practice", new Vector2(60, 144),
